...
<div class="card p-3">
    <h3 class="fw-bolder">Customer</h3>
    <hr>
    <table class="table table-hover tab-pane">
        ...
        @foreach($CustomData as $key => $item)
            <tr class="removeble-tr-{{ $item->id }}">
                ...
                <td><input type="text" class="border-0 update-address" data-id='{{$item->id}}'
...
<script>
    $(document).ready(function () {
        $(".update-name").change(function () {
            ...
            $.ajax({
                url: '{{ route('update.customer.name') }}',
                method: 'POST',
                data: {
                    id: id,
                    name: name,
        ...
        $(".update-address").change(function () {
            ...
            var id = $(this).attr('data-id');
            $.ajax({
                url: '{{ route('update.customer.address') }}',
                method: 'POST',
                data: {
                    id: id,
                    address: address,
